...
from PyQt6.QtGui import QIcon, QPixmap, QPainter, QPen, QColor, QImage, QFont
from PyQt6.QtWidgets import (
    QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
    QPushButton, QFileDialog, QLabel, QGroupBox, QInputDialog,
    QMessageBox, QProgressBar, QSpinBox, QDialog, QColorDialog,
...
class KayitKlasoruDialog(QDialog):
    def __init__(self, son_foto_tarih=None, parent=None):
        super().__init__(parent)
        self.setWindowTitle("📁 Kayıt Klasörü Bilgileri")
        ...
        if son_foto_tarih:
            print(f"🔍 Gelen tarih: {son_foto_tarih}")
            
            # 🔥 ÖNCE iki noktayı noktaya çevir (2026:04:30 → 2026.04.30)
            temiz_tarih = son_foto_tarih.replace(':', '.')
            print(f"   İki nokta dönüştürüldü: {temiz_tarih}")
            
            # Şimdi tarih formatını yakala (2026.04.30)
            tarih_match = re.search(r'(\d{4}\.\d{2}\.\d{2})', temiz_tarih)
            if tarih_match:
                self.tespit_tarihi = tarih_match.group(1)
                print(f"   ✅ Tarih bulundu: {self.tespit_tarihi}")
            else:
                self.tespit_tarihi = time.strftime("%Y.%m.%d")
                print(f"   ⚠️ Tarih bulunamadı, bugün kullanılıyor: {self.tespit_tarihi}")
        else:
            self.tespit_tarihi = time.strftime("%Y.%m.%d")
            print(f"   ⚠️ Tarih gelmedi, bugün kullanılıyor: {self.tespit_tarihi}")
        
        self.tarih_label = QLabel()
        self.tarih_label.setText(f"<b>{self.tespit_tarihi}</b>")
        self.tarih_label.setStyleSheet("color: #0078d7; font-size: 14px;")
        form_layout.addRow("📅 Tespit Tarihi (otomatik):", self.tarih_label)
        
        # Şebeke Unsuru - QLineEdit ile
        self.sebeke_unsuru = QLineEdit()
        self.sebeke_unsuru.setPlaceholderText("Örnek: 754475_H05 , 1405673_SEKSİYONER...")
        form_layout.addRow("🔌 Şebeke Unsuru:", self.sebeke_unsuru)
        
        # Şebeke Adı - QLineEdit ile
        self.sebeke_adi = QLineEdit()
        self.sebeke_adi.setPlaceholderText("Örnek: Kaş Sehirçam Dm Ges Çıkışı...")
        form_layout.addRow("📍 Şebeke Adı:", self.sebeke_adi)
        ...
        # Önizleme güncelleme
        self.sebeke_unsuru.textChanged.connect(self.update_preview)
        self.sebeke_adi.textChanged.connect(self.update_preview)
        
        self.update_preview()
    
    def update_preview(self):
        unsuru = self.sebeke_unsuru.text().strip()
        adi = self.sebeke_adi.text().strip()
        
        if unsuru and adi:
            klasor_adi = f"{self.tespit_tarihi}_{unsuru}_{adi}"
        elif unsuru:
            klasor_adi = f"{self.tespit_tarihi}_{unsuru}"
        elif adi:
            klasor_adi = f"{self.tespit_tarihi}_{adi}"
        else:
            klasor_adi = self.tespit_tarihi
        
        self.preview_label.setText(f"📁 Oluşturulacak Klasör:\n\n{klasor_adi}")
    
    def get_klasor_adi(self):
        unsuru = self.sebeke_unsuru.text().strip()
        adi = self.sebeke_adi.text().strip()
        
        parcalar = [self.tespit_tarihi]
        if unsuru:
            parcalar.append(unsuru)
        if adi:
            parcalar.append(adi)
...
class DroneHarita(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Drone Tespit - Halil ibrahim Başkaya")
        ...
        # TEK BUTON - İkisinin yerine
        self.foto_islem_buton = QPushButton("🖼 Birleştir + İsteğe Bağlı Küçült")
        self.foto_islem_buton.clicked.connect(self.fotograf_isle)
        resim_kucultme_layout.addWidget(self.foto_islem_buton)
    ...
    def en_yakina_isle(self):
        js_kod = """
            (function() {
                // SADECE dışarıdaki fotoğrafları bul (isAttached = false veya hiçbir direkte kayıtlı olmayan)
                const disaridakiFotoğraflar = photoInfo.filter(p => {
                    // Marker'ı olmayanları atla
                    if (!p.marker) return false;
                    
                    // isAttached flag'ine bak
                    if (p.isAttached === true) return false;
                    
                    // Çifte kontrol: Herhangi bir direkte bu fotoğraf var mı?
                    const direklerdeVarMi = direkler.some(d => 
                        d.photos && d.photos.some(f => f.path === p.path)
                    );
                    
                    if (direklerdeVarMi) {
                        // Flag'i düzelt
                        p.isAttached = true;
                        console.log(`🔧 DÜZELTİLDİ: ${p.filename} -> isAttached=true (direkte bulundu)`);
                        return false;
                    }
                ...
                console.log("=".repeat(60));
                console.log(`📸 TOPLAM FOTOĞRAF: ${photoInfo.length}`);
                console.log(`✅ İÇERİDEKİLER (manuel eklenen): ${photoInfo.filter(p => p.isAttached === true).length}`);
                console.log(`⬜ DIŞARIDAKİLER (işlenecek): ${disaridakiFotoğraflar.length}`);
                console.log("=".repeat(60));
                
                if (disaridakiFotoğraflar.length === 0) {
                    console.log("⚠️ Dışarıda fotoğraf kalmamış!");
                } else {
                    console.log("📋 DIŞARIDAKİ FOTOĞRAFLAR:");
                    disaridakiFotoğraflar.forEach(p => {
                        console.log(`   ⬜ ${p.filename}`);
                    });
                }
                
                if (photoInfo.filter(p => p.isAttached === true).length > 0) {
                    console.log("📋 İÇERİDEKİ FOTOĞRAFLAR (manuel eklenenler):");
                    photoInfo.filter(p => p.isAttached === true).forEach(p => {
                        console.log(`   ✅ ${p.filename}`);
                    });
                }
                
                return {
                    photoInfo: disaridakiFotoğraflar.map(p => {
                        let pos = p.marker.getLatLng();
                        return [p.path, p.id, pos.lat, pos.lng, p.note || "", p.tarih || ""];
                    }),
                    ...
                    filterStats: {
                        total: photoInfo.length,
                        disaridaki: disaridakiFotoğraflar.length,
                        icerideki: photoInfo.filter(p => p.isAttached === true).length
                    }
                };
            })();
        """
    ...
    def en_yakina_isle_guncelle(self, veri):
        ...
        disaridaki_fotograflar = veri["photoInfo"]
        
        filter_stats = veri.get("filterStats", {})
        total_before = filter_stats.get("total", 0)
        disaridaki_sayisi = filter_stats.get("disaridaki", 0)
        icerideki_sayisi = filter_stats.get("icerideki", 0)
        
        print("\n" + "="*60)
        print("📊 FİLTRELEME RAPORU")
        print("="*60)
        print(f"📸 Toplam fotoğraf: {total_before}")
        print(f"✅ İçerideki fotoğraflar (manuel eklenen, ATLANACAK): {icerideki_sayisi}")
        print(f"⬜ Dışarıdaki fotoğraflar (İŞLENECEK): {disaridaki_sayisi}")
        if total_before > 0:
            print(f"📈 Tasarruf oranı: %{(icerideki_sayisi/total_before)*100:.1f} (içeridekiler atlandı)")
        print("="*60 + "\n")
        
        # Dışarıdaki fotoğrafları tarihe göre sırala
        disaridaki_fotograflar.sort(key=lambda x: x[5] if len(x) > 5 and x[5] else "")
        
        # 🔥 KRİTİK: self.photo_info'yu ASLA güncelleme! Bu Python tarafını bozar
        # self.photo_info = disaridaki_fotograflar  # <- BU SATIRI KESİNLİKLE KULLANMA!

        if not disaridaki_fotograflar:
            QMessageBox.information(self, "İşlem Gerekmiyor", 
                f"✅ Tüm fotoğraflar zaten direklerin içinde!\n\n"
                f"📊 İçerideki fotoğraf sayısı: {icerideki_sayisi}\n"
                f"📸 Toplam fotoğraf: {total_before}\n\n"
                f"📍 Dışarıda işlenecek fotoğraf kalmamış.")
            self.kaydetme_bar.setVisible(False)
            return
        
        if not self.direkler:
            QMessageBox.warning(self, "Veri Eksik", "⚠️ Direk verisi eksik!")
            self.kaydetme_bar.setVisible(False)
            return

        if icerideki_sayisi > 0:
            QMessageBox.information(
                self, 
                "Filtreleme Aktif", 
                f"📊 {disaridaki_sayisi} fotoğraf işlenecek.\n\n"
                f"✅ {icerideki_sayisi} fotoğraf zaten direk içinde olduğu için ATLANDI.\n\n"
                f"💡 Bu sayede işlem süresi %{(icerideki_sayisi/total_before)*100:.1f} daha hızlı olacak."
            )
        ...
        print(f"📸 İşlenecek fotoğraf sayısı: {disaridaki_sayisi}")
        print("="*60)

        toplam_foto = len(disaridaki_fotograflar)
        self.kaydetme_bar.setMaximum(toplam_foto)
        self.kaydetme_bar.setValue(0)
        self.kaydetme_bar.setFormat(f"Fotoğraflar taranıyor... 0% (0/{toplam_foto})")
        QApplication.processEvents()

        for i, item in enumerate(disaridaki_fotograflar):
            if len(item) >= 6:
                foto_path, foto_id, foto_lat, foto_lon, note, tarih = item[:6]
            else:
                foto_path, foto_id, foto_lat, foto_lon, note = item[:5]
                tarih = ""
            ...
            if yakin_direkler:
                ...
                print(f"   ├─ EN YAKIN: {en_yakin_direk['id']} - {en_yakin_mesafe:.1f}m")
                
                kontrol_edilecek.append((foto_path, foto_id, en_yakin_direk, foto_lat, foto_lon, note, tarih))
            else:
                mesafe_disi += 1
                foto_adi = os.path.basename(foto_path)
                print(f"\n⚠️ {foto_adi} (DIŞARIDA - direk bulunamadı)")
                if tarih:
                    print(f"   ├─ Tarih : 🕒 {tarih}")
                print(f"   ├─ 30m içinde direk bulunamadı")

        print("\n" + "="*60)
        print(f"📊 TARAMA TAMAMLANDI")
        print(f"   • Dışarıdaki fotoğraf: {toplam_foto}")
        print(f"   • 30m içinde direk bulanan: {len(kontrol_edilecek)}")
        ...
        if kontrol_edilecek:
            ...
            def devam_et_kontrol(kalanlar, index=0):
                ...
                if index >= len(kalanlar):
                    ...
                    QMessageBox.information(self, "İşlem Tamamlandı",
                        ...
                        f"📊 SONUÇLAR:\n"
                        f"• Dışarıdaki fotoğraf: {toplam_foto}\n"
                        f"• İçeri alınan fotoğraf: {otomatik_iliskilenen}\n"
                        f"• Direk bulunamayan: {mesafe_disi}\n\n"
                        f"📍 Not: SADECE dışarıdaki fotoğraflar işlendi.\n"
                        f"✅ İçerideki {icerideki_sayisi} fotoğraf korundu.")
                    
                    self.view.page().runJavaScript("guncellePopuplar();")
                    return

                foto_path, foto_id, direk, foto_lat, foto_lon, note, tarih = kalanlar[index]
                ...
                def handle_kontrol(result):
                    ...
                    if result:
                        ...
                        self.view.page().runJavaScript(f"""
                            (function() {{
                                ...
                                if (p) {{
                                    ...
                                    console.log("Original kaydedildi:", fullPath);
                                }}
                                ...
                                if (p && p.marker && map.hasLayer(p.marker)) {{
                                    ...
                                    p.marker = null;
                                    p.isAttached = true;
                                    console.log("✅ DIŞARIDAKİ fotoğraf içeri alındı, isAttached=true:", fullPath);
                                }}
                            }})();
                        """)
                        ...
                        print(f"✅ DIŞARIDAKİ fotoğraf eklendi: {os.path.basename(foto_path)} -> {direk['id']}")
            ...
            QMessageBox.information(self, "İşlem Tamamlandı", 
                ...
                f"• Dışarıdaki fotoğraf: {toplam_foto}\n"
                f"• Direk bulunamayan: {mesafe_disi}\n\n"
                f"📍 Dışarıdaki hiçbir fotoğraf 30m içinde direk bulamadı.")
    ...
    def fotograflari_yukle(self):
        ...
        photo_data.sort(key=lambda x: x[3] if x[3] else "")
    ...
    def _son_fotograf_tarihini_bul(self):
        if not hasattr(self, 'photo_info') or not self.photo_info:
            return None
        
        en_son_tarih = None
        
        for foto in self.photo_info:
            if len(foto) >= 4 and foto[3]:
                tarih = str(foto[3])
                
                import re
                tarih_match = re.search(r'(\d{4}[-:]\d{2}[-:]\d{2})', tarih)
                if tarih_match:
                    temiz_tarih = tarih_match.group(1).replace(':', '.')
                    if en_son_tarih is None or temiz_tarih > en_son_tarih:
                        en_son_tarih = temiz_tarih
        
        if not en_son_tarih:
            en_son_tarih = time.strftime("%Y.%m.%d")
        
        print(f"📅 En son fotoğraf tarihi: {en_son_tarih}")
        return en_son_tarih
    ...
    def drone_kayit_klasoru_sec(self):
        ana_klasor = QFileDialog.getExistingDirectory(self, "Ana Kayıt Klasörünü Seçin (Örn: E:/2026/KAŞ)")
        
        if not ana_klasor:
            print("❌ Ana klasör seçilmedi")
            return
        
        son_tarih = self._son_fotograf_tarihini_bul()
        
        dialog = KayitKlasoruDialog(son_tarih, self)
        
        if dialog.exec() != QDialog.DialogCode.Accepted:
            print("🚫 Kullanıcı iptal etti")
            return
        
        alt_klasor_adi = dialog.get_klasor_adi()
        
        if not alt_klasor_adi:
            QMessageBox.warning(self, "Hata", "⚠️ Klasör adı oluşturulamadı!")
            return
        
        self.drone_kayit_klasoru = os.path.join(ana_klasor, alt_klasor_adi)
        
        try:
            os.makedirs(self.drone_kayit_klasoru, exist_ok=True)
            print(f"✅ Otomatik klasör oluşturuldu: {self.drone_kayit_klasoru}")
            ...
            QMessageBox.information(
                self,
                "Kayıt Klasörü Hazır",
                f"✅ Kayıt klasörü başarıyla hazırlandı!\n\n"
                f"📂 Ana Klasör: {ana_klasor}\n"
                f"📁 Alt Klasör: {alt_klasor_adi}\n"
                f"📌 Tam yol: {self.drone_kayit_klasoru}\n\n"
                f"Tüm kayıtlar bu klasör içine yapılacaktır."
            )
            
        except Exception as e:
            QMessageBox.critical(self, "Hata", f"Klasör oluşturulamadı:\n{str(e)}")
            self.drone_kayit_klasoru = ""
            self.kayit_label.setText("📁 Seçilen kayıt klasörü: Henüz seçilmedi")
    ...
    def fotograf_isle(self):
        """Tek buton - Kullanıcıya küçültme isteyip istemediğini sorar"""
        
        # 1. Kaynak klasör kontrolü
        if not self.foto_klasor:
            klasor = QFileDialog.getExistingDirectory(self, "📂 Kaynak Klasör Seçin (Fotoğrafların olduğu ana klasör)")
            if not klasor:
                return
            self.foto_klasor = klasor
            self.foto_klasor_label.setText(f"📁 {klasor}")
        ...
        if not self.foto_kayit_klasor:
            klasor = QFileDialog.getExistingDirectory(self, "💾 Hedef Klasör Seçin (Birleşen fotoğraflar buraya kaydedilecek)")
            if not klasor:
                return
            self.foto_kayit_klasor = klasor
            self.kayit_klasor_label.setText(f"📁 {klasor}")
        
        # 3. Kullanıcıya küçültme isteyip istemediğini sor
        msg = QMessageBox(self)
        msg.setWindowTitle("İşlem Seçimi")
        msg.setText("📸 Fotoğrafları nasıl işlemek istiyorsunuz?")
        msg.setIcon(QMessageBox.Icon.Question)
        
        kucult_btn = msg.addButton("🔄 Küçülterek Birleştir", QMessageBox.ButtonRole.YesRole)
        kopyala_btn = msg.addButton("📋 Orijinal Boyutta Birleştir", QMessageBox.ButtonRole.NoRole)
        iptal_btn = msg.addButton("❌ İptal", QMessageBox.ButtonRole.RejectRole)
        ...
        kuculterek_mi = msg.clickedButton() == kucult_btn
        print(f"📝 Seçim: {'Küçülterek' if kuculterek_mi else 'Orijinal boyutta'} birleştirilecek")
        
        # 4. Küçültme için kalite ayarları
        kalite_ayarlari = None
        if kuculterek_mi:
            dialog = KaliteAyarlariDialog(self)
            if dialog.exec() != QDialog.DialogCode.Accepted:
        ...
        # 5. Kullanıcıdan isim kökü iste
        isim_koku, ok = QInputDialog.getText(
            self, 
            "Dosya Adlandırma", 
            "📷 Fotoğrafların isimlendirileceği kök adı girin:\n\n"
            "Örnek: DJI → DJI_1, DJI_2, DJI_3 ...\n"
            "Örnek: Foto → Foto_1, Foto_2, Foto_3 ...\n\n"
            "Boş bırakırsanız orijinal adları korunur."
        )
        ...
        # 6. Tüm fotoğrafları topla (alt klasörler dahil)
        foto_uzantilari = ('.jpg', '.jpeg', '.png', '.bmp', '.webp', '.tif', '.tiff')
        tum_fotograflar = []
        
        print("🔍 Fotoğraflar taranıyor (alt klasörler dahil)...")
        
        for root, _, files in os.walk(self.foto_klasor):
            for file in files:
                if file.lower().endswith(foto_uzantilari):
                    tam_yol = os.path.join(root, file)
                    
                    # EXIF'ten çekim tarihini al (sıralama için)
                    tarih = None
                    try:
                        with Image.open(tam_yol) as img:
                            exif = img._getexif()
                            if exif:
                                tarih_tag = exif.get(36867) or exif.get(306)
                                if tarih_tag:
                                    tarih = str(tarih_tag)
                    except:
                        pass
        ...
        if not tum_fotograflar:
            QMessageBox.warning(self, "Fotoğraf Bulunamadı", 
                              f"⚠️ Seçilen klasörde uygun fotoğraf bulunamadı.\n\nKlasör: {self.foto_klasor}")
            return
        
        print(f"✅ Toplam {len(tum_fotograflar)} fotoğraf bulundu")
        
        # 7. Tarihe göre sırala (eskiden yeniye)
        tum_fotograflar.sort(key=lambda x: x['tarih'])
        
        # 8. Hedef klasörü oluştur
        os.makedirs(self.foto_kayit_klasor, exist_ok=True)
        
        # 9. İşlem çubuğunu göster
        self.kucultme_bar.setVisible(True)
        self.kucultme_bar.setMaximum(len(tum_fotograflar))
        self.kucultme_bar.setValue(0)
        self.kucultme_bar.setFormat("Hazırlanıyor... 0%")
        
        islem_adi = "küçültülüyor" if kuculterek_mi else "kopyalanıyor"
        ...
        for i, foto in enumerate(tum_fotograflar, start=1):
            try:
                kaynak = foto['tam_yol']
                
                # Yeni dosya adını oluştur
                if isim_koku and isim_koku.strip():
                    uzanti = os.path.splitext(foto['dosya_adi'])[1]
                    yeni_isim = f"{isim_koku}_{i}{uzanti}"
                else:
                    yeni_isim = foto['dosya_adi']
                
                hedef = os.path.join(self.foto_kayit_klasor, yeni_isim)
                
                orjinal_boyut = os.path.getsize(kaynak)
                toplam_orjinal_boyut += orjinal_boyut
                
                if kuculterek_mi and kalite_ayarlari:
                ...
                print(f"✅ {i}/{len(tum_fotograflar)}: {foto['dosya_adi']} → {yeni_isim}")
                
            except Exception as e:
                hata_sayisi += 1
                print(f"❌ {i}/{len(tum_fotograflar)}: {foto['dosya_adi']} - HATA: {e}")
            
            ilerleme = int(i / len(tum_fotograflar) * 100)
            self.kucultme_bar.setValue(i)
            self.kucultme_bar.setFormat(f"{islem_adi}... {ilerleme}% ({i}/{len(tum_fotograflar)})")
            QApplication.processEvents()
        
        # 11. Excel dosyalarını da kopyala (varsa)
        excel_kopyalandi = 0
        try:
            for root, _, files in os.walk(self.foto_klasor):
                for file in files:
                    if file.lower().endswith((".xlsx", ".xls")):
                        shutil.copy2(os.path.join(root, file), self.foto_kayit_klasor)
                        excel_kopyalandi += 1
        except:
            pass
        
        # 12. İstatistikleri göster
        self.kucultme_bar.setFormat("✅ Tamamlandı")
        
        orjinal_mb = toplam_orjinal_boyut / (1024 * 1024)
        yeni_mb = toplam_yeni_boyut / (1024 * 1024)
        tasarruf_orani = ((orjinal_mb - yeni_mb) / orjinal_mb * 100) if orjinal_mb > 0 else 0
        
        islem_turu = "Küçültme" if kuculterek_mi else "Kopyalama"
        kalite_bilgisi = ""
        if kuculterek_mi and kalite_ayarlari:
            kalite_bilgisi = f"\n\n⚙️ KULLANILAN AYARLAR:\n• Maksimum boyut: {kalite_ayarlari['max_width']}x{kalite_ayarlari['max_height']} px\n• Kalite: {kalite_ayarlari['quality']}%"
        
        QMessageBox.information(
            self, 
            f"{islem_turu} İşlemi Tamamlandı",
            f"✅ {islem_turu} işlemi başarıyla tamamlandı!\n\n"
            f"📊 İSTATİSTİKLER:\n"
            f"• Toplam fotoğraf: {len(tum_fotograflar)}\n"
            f"• Başarılı: {basarili_sayisi}\n"
            f"• Hatalı: {hata_sayisi}\n\n"
            f"📊 BOYUT KARŞILAŞTIRMASI:\n"
            f"• Orijinal boyut: {orjinal_mb:.2f} MB\n"
            f"• Yeni boyut: {yeni_mb:.2f} MB\n"
            f"• Kazanılan alan: {(orjinal_mb - yeni_mb):.2f} MB\n"
            f"• Tasarruf oranı: %{tasarruf_orani:.1f}"
            f"{kalite_bilgisi}\n\n"
            f"📝 İSİMLENDİRME:\n"
            f"• Kök ad: {isim_koku if isim_koku else '(orijinal adlar korundu)'}\n"
            f"📄 Excel dosyası: {excel_kopyalandi} adet kopyalandı\n\n"
            f"📁 Hedef klasör:\n{self.foto_kayit_klasor}"
        )
...
def create_map(photo_data, direk_data, save_path, center_lat=None, center_lon=None, zoom_level=None):
    ...
    <body>
        ...
        <script>
            ...
            function getPhotoPopupHTML(path, note, birim = "", tarih = "") {{
                ...
                const tarihHtml = tarih ? `<div style="font-size: 10px; color: #666; margin-bottom: 5px;">🕒 ${{tarih}}</div>` : "";
                
                return `
                    <div style='width: 280px; max-width: 98%; line-height: 1.2;'>
                        <b style="word-break: break-all; font-size: 12px; margin: 0; display: block;">📁 ${{displayPath}}</b>
                        ${{tarihHtml}}
                        <img src='${{path}}' style='width: 98%; max-width: 280px; border-radius: 3px; box-shadow: 0 0 3px #0002; margin: 2px 0; display: block;'>
                        ...
                        <div style="margin: 3px 0 2px 0;">
                            ...
                            <textarea id="note-input-${{encoded}}" 
                                      style="width: 98%; max-width: 280px; 
                                             padding: 4px;
                                             margin-top: 2px;
            ...
            function direkSilindiktenSonraNumaralariDuzenle(silinenNumara) {{
                var buyukNumaraliDirekler = direkler
                    .filter(d => d.numara !== null && d.numara > silinenNumara)
                    .sort((a, b) => a.numara - b.numara);
                
                var sira = silinenNumara;
                buyukNumaraliDirekler.forEach(direk => {{
                    direk.numara = sira;
                ...
                if (buyukNumaraliDirekler.length > 0) {{
                    console.log(`🔢 Direk silindi, numaralar yeniden düzenlendi: ${{silinenNumara}} ve üstü`);
                }}
            }}
            ...
            function kaldirDirek(direkId) {{
                ...
                const silinenNumara = direk.numara;
                    ...
                    direk.photos.forEach(foto => {{
                        ...
                        if (photoInfoObj) {{
                            ...
                            var marker = L.marker([lat, lon], {{
                                ...
                                zIndexOffset: 800
                            }}).bindPopup(getPhotoPopupHTML(photoInfoObj.path, photoInfoObj.note, photoInfoObj.birim, photoInfoObj.tarih), {{
                                maxWidth: 350,
                                minWidth: 280,
                                autoPan: true,
                ...
                if (silinenNumara) {{
                    direkSilindiktenSonraNumaralariDuzenle(silinenNumara);
                }}
            ...
            function tumFotolariCikar() {{
                var minNumara = prompt("📸 Fotoğrafların dışarı alınacağı minimum sıra numarasını girin:", "1");
                ...
                var minNumaraInt = parseInt(minNumara);
                if (isNaN(minNumaraInt)) {{
                    alert("❌ Geçerli bir sayı girmediniz!");
                    return;
                }}
                
                console.log("📊 " + minNumaraInt + " ve üzeri sıra numaralı direklerin fotoğrafları dışarı alınıyor...");
                
                var tumFotograflar = [];
                var etkilenenDirekSayisi = 0;
                var toplamFotografSayisi = 0;
                
                for (var i = 0; i < direkler.length; i++) {{
                    var direk = direkler[i];
                    if (direk.numara && direk.numara >= minNumaraInt && direk.photos && direk.photos.length > 0) {{
                        etkilenenDirekSayisi++;
                        toplamFotografSayisi += direk.photos.length;
                        
                        for (var j = 0; j < direk.photos.length; j++) {{
                            var foto = direk.photos[j];
                            tumFotograflar.push({{
                                path: foto.path,
                                direk_id: direk.id,
                                direk_numara: direk.numara,
                                note: foto.note || ""
                            }});
                        }}
                ...
                if (tumFotograflar.length === 0) {{
                    alert("⚠️ " + minNumaraInt + " ve üzeri sıra numarasına sahip direk bulunamadı veya bu direklerin fotoğrafı yok!");
                    return;
                }}
                
                console.log("📸 " + toplamFotografSayisi + " fotoğraf " + etkilenenDirekSayisi + " direkten çıkarılıyor...");
                
                for (var k = 0; k < tumFotograflar.length; k++) {{
                    var foto = tumFotograflar[k];
                    var photoInfoObj = null;
                    
                    for (var m = 0; m < photoInfo.length; m++) {{
                        if (photoInfo[m].path === foto.path) {{
                            photoInfoObj = photoInfo[m];
                            break;
                        }}
                    }}
                    
                    if (photoInfoObj) {{
                        var lat = (photoInfoObj.originalLat !== undefined && photoInfoObj.originalLat !== null) ? photoInfoObj.originalLat : photoInfoObj.lat;
                        var lon = (photoInfoObj.originalLon !== undefined && photoInfoObj.originalLon !== null) ? photoInfoObj.originalLon : photoInfoObj.lon;
                        
                        console.log("📍 Fotoğraf orijinal konumuna dönüyor: " + foto.path);
                        ...
                        marker.bindPopup(getPhotoPopupHTML(photoInfoObj.path, photoInfoObj.note, photoInfoObj.birim, photoInfoObj.tarih), {{
                            maxWidth: 350,
                            minWidth: 280,
                            autoPan: true,
                            className: 'custom-popup'
                        ...
                        marker.on("contextmenu", function(fotoPath, fotoNote, fotoKategori, fotoOncelik, fotoBirim, fotoTarih) {{
                            return function(e) {{
                                e.originalEvent.preventDefault();
                                e.originalEvent.stopPropagation();
                                ...
                                for (var idx = 0; idx < direkler.length; idx++) {{
                                    var d = direkler[idx];
                                    var mesafe = map.distance(pos, L.latLng(d.lat, d.lon));
                                    if (mesafe < 40 && mesafe < enKisaMesafe) {{
                                        hedefDirek = d;
                                ...
                                if (hedefDirek) {{
                                    var zatenVar = false;
                                    for (var idx2 = 0; idx2 < hedefDirek.photos.length; idx2++) {{
                                        if (hedefDirek.photos[idx2].path === fotoPath) {{
                                            zatenVar = true;
                                            break;
                                        }}
                                    }}
                                    ...
                                    if (hedefDirek.photos.length === 1 && !hedefDirek.numara) {{
                                        hedefDirek.numara = getNextAvailableNumber();
                        ...
                        }}(photoInfoObj.path, photoInfoObj.note || "", photoInfoObj.kategori || "", photoInfoObj.oncelik || "Bekleyebilir", photoInfoObj.birim || "Aob", photoInfoObj.tarih || ""));
                        
                        marker.on("popupopen", function(path) {{
                            return function() {{
                                bindNoteInput(path);
                            }};
                        }}(photoInfoObj.path));
                        
                        marker.on("dragend", function(fotoPath, fotoNote, fotoKategori, fotoOncelik, fotoBirim, fotoTarih) {{
                            return function(e) {{
                                var pos = e.target.getLatLng();
                                var hedefDirek = null;
                                var enKisaMesafe = Infinity;
                                ...
                                for (var idx = 0; idx < direkler.length; idx++) {{
                                    ...
                                    if (mesafe < 8 && mesafe < enKisaMesafe) {{
                                        var zatenVar = false;
                                        for (var idx2 = 0; idx2 < d.photos.length; idx2++) {{
                                            if (d.photos[idx2].path === fotoPath) {{
                                                zatenVar = true;
                                                break;
                                            }}
                                        }}
                                ...
                                if (hedefDirek) {{
                                    if (photoInfoObj.originalLat === undefined || photoInfoObj.originalLon === undefined) {{
                                        photoInfoObj.originalLat = photoInfoObj.lat;
                                        photoInfoObj.originalLon = photoInfoObj.lon;
                                    }}
                                    
                                    photoInfoObj.lat = pos.lat;
                                    photoInfoObj.lon = pos.lng;
                ...
                for (var n = 0; n < direkler.length; n++) {{
                    guncelleDirekRengi(direkler[n]);
                    guncellePopup(direkler[n]);
                }}
                
                alert("✅ İşlem tamamlandı!\\n\\n📊 İSTATİSTİKLER:\\n• Etkilenen direk sayısı: " + etkilenenDirekSayisi + "\\n• Dışarı alınan fotoğraf: " + toplamFotografSayisi + "\\n• Minimum sıra numarası: " + minNumaraInt);
            }}
            ...
            var TumFotolariCikarControl = L.Control.extend({{
                ...
                onAdd: function(map) {{
                    ...
                    link.href = '#';
                    link.innerHTML = '🔄';
                    link.title = 'Sıra numarasına göre fotoğrafları dışarı al';
            ...
            function kaydetNumara(id) {{
                ...
                const eskiNumara = d.numara;
                
                if (eskiNumara === yeniNumara) {{
                    return;
                }}

                const ayniNumaraVar = direkler.some(x => x.numara === yeniNumara && x.id !== id);
                if (ayniNumaraVar) {{
                    alert(`❌ ${{yeniNumara}} numarası zaten başka bir direkte kullanılıyor!\\n\\nDirek ID: ${{direkler.find(x => x.numara === yeniNumara).id}}`);
                    input.value = eskiNumara ?? "";
                    return;
                }}

                const etkilenecekDirekler = direkler
                    .filter(diger => diger.numara !== null && diger.numara > eskiNumara && diger.id !== id)
                    .sort((a, b) => a.numara - b.numara);
                
                let mesaj = `🔢 Direk numarası değiştirilecek:\\n\\n`;
                mesaj += `${{d.id}}: ${{eskiNumara}} → ${{yeniNumara}}\\n\\n`;
                
                if (etkilenecekDirekler.length > 0) {{
                    mesaj += `⚠️ Aşağıdaki ${{etkilenecekDirekler.length}} direk de etkilenecek:\\n`;
                    etkilenecekDirekler.forEach((diger, idx) => {{
                        if (idx < 5) {{
                            mesaj += `   • ${{diger.id}}: ${{diger.numara}} → ${{yeniNumara + (diger.numara - eskiNumara)}}\\n`;
                        }}
                    }});
                    if (etkilenecekDirekler.length > 5) {{
                        mesaj += `   ... ve ${{etkilenecekDirekler.length - 5}} direk daha\\n`;
                    }}
                    mesaj += `\\n`;
                }}
                ...
                let offset = yeniNumara - eskiNumara;
                etkilenecekDirekler.forEach(diger => {{
                    const yeniDeger = diger.numara + offset;
                    diger.numara = yeniDeger;
                    guncelleDirekNumaraGorunumu(diger);
                    guncellePopup(diger);
                    console.log(`🔢 ${{diger.id}}: ${{diger.numara - offset}} → ${{yeniDeger}}`);
                }});
                ...
                setTimeout(() => {{
                    let sonucMesaj = `✅ Numaralandırma tamamlandı!\\n\\n`;
                    sonucMesaj += `📌 ${{d.id}}: ${{eskiNumara}} → ${{yeniNumara}}\\n`;
                    if (etkilenecekDirekler.length > 0) {{
                        sonucMesaj += `📌 ${{etkilenecekDirekler.length}} direk otomatik güncellendi\\n`;
                        sonucMesaj += `📌 Aralık: ${{yeniNumara + 1}} - ${{yeniNumara + etkilenecekDirekler.length}}`;
                    }}
                    
                    const popup = L.popup()
                        .setLatLng(d.marker.getLatLng())
                        .setContent(sonucMesaj)
                        .openOn(map);
                    setTimeout(() => map.closePopup(popup), 3000);
                }}, 100);
                
                console.log(`✅ Numaralandırma: ${{d.id}} ${{eskiNumara}}→${{yeniNumara}}, ${{etkilenecekDirekler.length}} direk güncellendi`);
            }}
            ...
            function addPhotoToDirek(direkId, fullPath) {{
                ...
                if (foto) {{
                    ...
                    foto.isAttached = true;
                    console.log(`📍 Fotoğraf marker'ı kaldırıldı, isAttached=true: ${{fullPath}}`);
                }} else {{
                    console.warn(`⚠️ PhotoInfo içinde fotoğraf bulunamadı: ${{fullPath}}`);
                }}
                
                if (direk.photos.length === 1 && (typeof direk.numara === 'undefined' || direk.numara === null)) {{
                    direk.numara = getNextAvailableNumber();
                    console.log(`🔢 Yeni numara atandı: ${{direk.numara}} -> ${{direkId}}`);
                }}
                ...
                console.log(`✅ Fotoğraf eklendi: ${{fullPath}} -> ${{direkId}} (Numara: ${{direk.numara}}, isAttached: ${{foto?.isAttached}})`);
                
                const unattachedCount = photoInfo.filter(p => !p.isAttached).length;
                console.log(`📊 Kalan eklenmemiş fotoğraf: ${{unattachedCount}}`);
            }}
            ...
            function geriCagir(fullPath) {{
                ...
                var marker = L.marker([lat, lon], {{
                    ...
                    zIndexOffset: 800
                }}).bindPopup(getPhotoPopupHTML(foto.path, foto.note || "", foto.birim || "", foto.tarih || ""), {{
                    maxWidth: 350,
                    minWidth: 280,
                    autoPan: true,
                    className: 'custom-popup'
                ...
                foto.marker = marker;
                map.addLayer(marker);
                console.log(`📍 Marker EXIF konumuna geri döndü: ${{foto.path}}, isAttached=${{foto.isAttached}}`);
            }}
    """
        ...
        for path, lat, lon, tarih, photo_id in photo_data:
            ...
            photo_markers += f"""
                (function() {{
                    ...
                    marker.bindPopup(getPhotoPopupHTML(fullPath, "", "", tarih), {{
                        maxWidth: 350,
                        minWidth: 280,
                        autoPan: true,
                        className: 'custom-popup'
                    ...
                    marker.on("contextmenu", function(e) {{
                        ...
                        if (hedefDirek) {{
                            ...
                            if (hedefDirek.photos.length === 1 && !hedefDirek.numara) {{
                                hedefDirek.numara = getNextAvailableNumber();
                                console.log(`Yeni numara atandı: ${{hedefDirek.numara}}`);
                                guncelleDirekNumaraGorunumu(hedefDirek);
                            }}
                    ...
                    marker.on("popupopen", function() {{
                        ...
                        const guncelTarih = foto?.tarih || "";
                        
                        marker.setPopupContent(getPhotoPopupHTML(fullPath, guncelNote, guncelBirim, guncelTarih));
...
class FotoEditorViewer(QMainWindow):
    def __init__(self, foto_verileri, view=None, parent=None):
        ...
        # HER FOTOĞRAF İÇİN AYRI ÇİZİM LİSTELERİ
        self.all_drawings = {}
        self.all_texts = {}
...
class FotoEditorCanvas(QWidget):
    def __init__(self, image_path, parent=None):
        ...
        # HER FOTOĞRAF İÇİN AYRI BİR ID OLUŞTUR
        self.photo_id = hashlib.md5(image_path.encode()).hexdigest()[:16]
        ...
        # ÇİZİMLERİ FOTOĞRAF ID'SİNE GÖRE YÜKLE
        if hasattr(parent, 'all_drawings'):
            self.drawings = parent.all_drawings.get(self.photo_id, [])
            self.texts = parent.all_texts.get(self.photo_id, [])
        else:
            self.drawings = []
            self.texts = []
    ...
    def kaydet(self):
        try:
            ...
            if self.editor_viewer:
                if not hasattr(self.editor_viewer, 'all_drawings'):
                    self.editor_viewer.all_drawings = {}
                    self.editor_viewer.all_texts = {}
                
                self.editor_viewer.all_drawings[self.photo_id] = self.drawings.copy()
                self.editor_viewer.all_texts[self.photo_id] = self.texts.copy()
            ...
            if hasattr(self.editor_viewer, "note_input") and hasattr(self.editor_viewer, "view"):
                ...
                if not_metni:
                    ...
                    not_metni_clean = not_metni.replace("'", "\\'").replace('"', '\\"')
                    js = f"""
                        (function() {{
                            const photo = photoInfo.find(p => p.path === "{js_path}");
                            if (photo) {{
                                photo.note = `{not_metni_clean}`;
                ...
                if self.editor_viewer:
                    mevcut_index = self.editor_viewer.index
                    if mevcut_index < len(self.editor_viewer.fotolar):
                        self.editor_viewer.fotolar[mevcut_index]['note'] = not_metni

            # ========== SADECE SES ÇAL ==========
            winsound.MessageBeep(winsound.MB_ICONASTERISK)  # "Bing" sesi
            # ====================================

        except Exception as e:
            print(f"Kayıt hatası: {e}")
            winsound.MessageBeep(winsound.MB_ICONHAND)  # Hata sesi
