...
class ExcelSecimDialog(tk.Toplevel):
    def __init__(self, parent, nihai_direk_eklendi=False):  # Yeni parametre EKLENDİ
        super().__init__(parent)
        self.parent = parent
        self.nihai_direk_eklendi = nihai_direk_eklendi  # Yeni değişken
...
class AnaUygulama:
    ...
    # ========== YENİ EKLENEN METOD (BURAYA) ==========
    def metni_normalize_et(self, metin):
        """Metindeki birden fazla boşluğu tek boşluğa düşürür, baştaki ve sondaki boşlukları temizler"""
        import re
    ...
    def tespit_bilgilerini_birlestir(self, foto_klasorleri):
        """Tespit Excel dosyalarını birleştirir ve döndürür - KÖPRÜ formülleri için DÜZELTİLMİŞ"""
        tespit_verileri = []
        ...
        try:
            ...
            for dosya in excel_dosyalari:
                try:
                    ...
                    dosya_klasor_baslik = os.path.basename(dosya_klasoru)
                    ilce_adi = self.aob_adi_bul(dosya_klasoru)
                    
                    self.log_mesaj_ekle(f"🔍 {os.path.basename(dosya)} - İlçe: {ilce_adi}, Hat: {dosya_klasor_baslik}")
                    
                    # ✅ KRİTİK DÜZELTME: openpyxl ile formülleri ÇALIŞTIRMADAN oku
                    from openpyxl import load_workbook
                    
                    wb = load_workbook(dosya, data_only=False)  # data_only=False -> formülleri olduğu gibi al
                    ws = wb.active
                    ...
                    satirlar = []
                    for row_idx in range(1, min(max_row + 1, 20)):  # İlk 20 satırı kontrol et
                        satir = []
                        for col_idx in range(1, max_col + 1):
                            hucre = ws.cell(row=row_idx, column=col_idx)
                            satir.append(hucre.value if hucre.value is not None else "")
                        satirlar.append((row_idx, satir))
                    
                    # Başlık satırını bul
                    for row_idx, satir in satirlar:
                        satir_metin = ' '.join(str(x).strip() for x in satir if x and str(x).strip())
                        
                        # İstenen sütun isimlerini ara
                        eslesme_sayisi = 0
                        for sutun_adi in istenen_sutunlar:
                            if sutun_adi == 'Hat Adı':
                                continue
                            if any(sutun_adi.lower() in str(x).lower() for x in satir if x):
                                eslesme_sayisi += 1
                        
                        # Direk No/ID kontrolü
                        direk_var = any(any(dg.lower() in str(x).lower() for x in satir if x) 
                                       for dg in direk_no_degiskenleri + direk_id_degiskenleri)
                        
                        if eslesme_sayisi >= 3 and direk_var:  # En az 3 eşleşme + direk varsa
                            baslik_satiri_idx = row_idx
                            self.log_mesaj_ekle(f"   ✓ Başlık satırı {row_idx}. satırda bulundu")
                            break
                    
                    if baslik_satiri_idx is None:
                        # Alternatif: ilk satırı başlık olarak kullan
                        baslik_satiri_idx = 1
                        self.log_mesaj_ekle(f"   ℹ️ Başlık bulunamadı, 1. satır başlık olarak kullanıldı")
                    
                    # Başlık satırını al
                    basliklar = []
                    for col_idx in range(1, max_col + 1):
                        hucre = ws.cell(row=baslik_satiri_idx, column=col_idx)
                        basliklar.append(str(hucre.value).strip() if hucre.value else f"Column_{col_idx}")
                    ...
                    for idx, baslik in enumerate(basliklar):
                        ...
                        # Direk No/ID sütunu
                        if 'direk' in baslik_lower and ('no' in baslik_lower or 'id' in baslik_lower or 'numara' in baslik_lower):
                            sutun_eslesmeleri[idx] = 'Direk No'
                            self.log_mesaj_ekle(f"   ✓ 'Direk No' sütunu: {baslik} (Col {idx+1})")
                        
                        # Diğer sütunlar
                        for istenen in istenen_sutunlar:
                            if istenen == 'Hat Adı':
                                continue
                            if istenen.lower() == baslik_lower or istenen.lower() in baslik_lower:
                                sutun_eslesmeleri[idx] = istenen
                                self.log_mesaj_ekle(f"   ✓ '{istenen}' sütunu: {baslik} (Col {idx+1})")
                                break
                    
                    # Veri satırlarını oku (başlık satırından sonra)
                    dosya_veri_sayisi = 0
                    
                    for row_idx in range(baslik_satiri_idx + 1, max_row + 1):
                        try:
                            tespit_data = {
                                'İlçe': ilce_adi,
                            ...
                            # Direk No bilgisini al
                            direk_degeri = ""
                            direk_col = None
                            for col_idx, sutun_adi in sutun_eslesmeleri.items():
                                if sutun_adi == 'Direk No':
                                    direk_col = col_idx
                                    break
                            
                            if direk_col is not None:
                                cell = ws.cell(row=row_idx, column=direk_col + 1)
                                direk_degeri = str(cell.value) if cell.value is not None else ""
                                if direk_degeri:
                            ...
                            if not tespit_data.get('Direk No'):
                                continue
                            
                            # ✅ KRİTİK: Fotoğraf Yolu için FORMÜL MÜ? METİN Mİ? KONTROL ET
                            foto_yolu_degeri = ""
                            
                            # Fotoğraf Yolu sütununu bul
                            foto_col = None
                            for col_idx, sutun_adi in sutun_eslesmeleri.items():
                                if sutun_adi == 'Fotoğraf Yolu':
                                    foto_col = col_idx
                                    break
                            
                            if foto_col is not None:
                                cell = ws.cell(row=row_idx, column=foto_col + 1)
                                cell_value = cell.value
                                
                                print(f"\n📸 {row_idx}. SATIR FOTOĞRAF YOLU:")
                                print(f"   Cell type: {type(cell_value)}")
                                print(f"   Cell value: '{cell_value}'")
                                print(f"   Cell data_type: {cell.data_type}")
                                
                                if cell.data_type == 'f':  # FORMÜL
                                    # Formül metnini al
                                    formula = cell.value
                                    print(f"   🔍 FORMÜL TESPİT EDİLDİ: '{formula}'")
                                    
                                    # Formülden görünen kısmı çıkar
                                    foto_yolu_degeri = self.parse_excel_hyperlink(formula)
                                    print(f"   🔗 Parse sonucu: '{foto_yolu_degeri}'")
                                else:
                                    # Düz metin
                                    foto_yolu_degeri = str(cell_value) if cell_value else ""
                                    print(f"   📝 Düz metin: '{foto_yolu_degeri}'")
                                
                                # Normalize et
                                foto_yolu_degeri_normalize = self.metni_normalize_et(foto_yolu_degeri)
                                print(f"   Normalize: '{foto_yolu_degeri_normalize}'")
                            else:
                                foto_yolu_degeri = ""
                            
                            # Diğer sütunları al
                            for col_idx, sutun_adi in sutun_eslesmeleri.items():
                                if sutun_adi in ['Direk No', 'Fotoğraf Yolu']:
                                    continue
                                
                                cell = ws.cell(row=row_idx, column=col_idx + 1)
                                deger = cell.value if cell.value is not None else ""
                                
                                # Yapıldı mı? için özel işlem
                                if sutun_adi == 'Yapıldı mı?':
                                    if deger:
                                        if isinstance(deger, (int, float)):
                                            deger = "EVET" if deger == 1 else "HAYIR"
                                        elif isinstance(deger, bool):
                                            deger = "EVET" if deger else "HAYIR"
                                
                                tespit_data[sutun_adi] = deger if pd.notna(deger) else ""
                            ...
                            if tespit_data['Direk No'] and ilce_adi and dosya_klasor_baslik:
                                ...
                                print(f"   🔑 Aranan anahtar: {anahtar}")
                                
                                if anahtar in direk_foto_eslestirme:
                                    foto_listesi = direk_foto_eslestirme[anahtar]
                                    print(f"   📁 Bu direk için {len(foto_listesi)} fotoğraf bulundu")
                                    
                                    if foto_yolu_degeri_normalize:
                                        print(f"   🎯 Hedef dosya adı: '{foto_yolu_degeri_normalize}'")
                                        
                                        for foto_adi, foto_tam_yol in foto_listesi:
                                            foto_adi_normalize = self.metni_normalize_et(foto_adi)
                                            if foto_adi_normalize == foto_yolu_degeri_normalize:
                                                tam_foto_yolu = foto_tam_yol
                                                print(f"      ✅ TAM EŞLEŞME BULUNDU! -> {foto_adi}")
                                                break
                                    
                                    if not tam_foto_yolu and foto_listesi:
                                        tam_foto_yolu = foto_listesi[0][1]
                                        print(f"      📸 FALLBACK: İlk fotoğraf kullanıldı: {os.path.basename(tam_foto_yolu)}")
                            
                            tespit_data['Fotoğraf Yolu'] = tam_foto_yolu if tam_foto_yolu else foto_yolu_degeri
                            
                            # Birim filtresi
                            birim_str = str(tespit_data.get('Birim', '')).strip().lower()
                            if not birim_str or birim_str in ['aob', 'nan', 'none', '']:
                                tespit_verileri.append(tespit_data)
                                dosya_veri_sayisi += 1
                            
                            if dosya_veri_sayisi <= 3:
                                self.log_mesaj_ekle(f"   📝 Örnek {dosya_veri_sayisi}: Direk={tespit_data['Direk No']}, Foto={os.path.basename(tespit_data['Fotoğraf Yolu']) if tespit_data['Fotoğraf Yolu'] else 'YOK'}")
                        
                        except Exception as e:
                            self.log_mesaj_ekle(f"   ⚠️ Satır {row_idx} işleme hatası: {str(e)}")
                            continue
                    
                    self.log_mesaj_ekle(f"✅ {os.path.basename(dosya)} - {dosya_veri_sayisi} tespit eklendi")
                    wb.close()
    ...
    def parse_excel_hyperlink(self, formula_text):
        """
        Excel KÖPRÜ formülünü parse eder - TIRNAKLARI TEMİZLER
        """
        print(f"\n🔍 PARSE BAŞLANGIÇ: '{formula_text}'")
        ...
        # KÖPRÜ formülü mü?
        if formula_text.upper().startswith('=KÖPRÜ(') or formula_text.upper().startswith('=HYPERLINK('):
            try:
                # =HYPERLINK("yol", "görünen") formatı için
                if ',' in formula_text:
                    parts = formula_text.split(',')
                    if len(parts) >= 2:
                        görünen_kısım = parts[1].strip()
                        
                        # Parantezi temizle
                        if görünen_kısım.endswith(')'):
                            görünen_kısım = görünen_kısım[:-1]
                        
                        # Tırnakları temizle (baştaki ve sondaki)
                        if görünen_kısım.startswith('"') and görünen_kısım.endswith('"'):
                            görünen_kısım = görünen_kısım[1:-1]
                        
                        print(f"   ✅ Parse sonucu (temiz): '{görünen_kısım}'")
                        return görünen_kısım
                
                # =KÖPRÜ("yol"; "görünen") formatı için
                if ';' in formula_text:
                    parts = formula_text.split(';')
                    if len(parts) >= 2:
                        görünen_kısım = parts[1].strip()
                ...
                # Regex ile dene
                import re
                # =HYPERLINK("yol", "görünen") veya =KÖPRÜ("yol"; "görünen")
                pattern = r'=(?:HYPERLINK|KÖPRÜ)\s*\(\s*"[^"]+"\s*[,;]\s*"([^"]+)"\s*\)'
                match = re.search(pattern, formula_text, re.IGNORECASE)
                if match:
                    print(f"   ✅ Regex sonucu: '{match.group(1)}'")
