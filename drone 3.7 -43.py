    ...
    def en_yakina_isle_guncelle(self, veri):
        ...
        for i, (foto_path, foto_id, foto_lat, foto_lon, note) in enumerate(updated_photo_info):
            ...
            if yakin_direkler:
                ...
                kontrol_edilecek.append((foto_path, foto_id, en_yakin_direk))
            else:
                mesafe_disi += 1
        ...
        if kontrol_edilecek:
            ...
            def devam_et_kontrol(kalanlar, index=0):
                ...
                foto_path, foto_id, direk = kalanlar[index]
                ...
                js_kontrol = f"""
                    (function() {{
                        ...
                        const fullPath = "{foto_path.replace('\\', '/')}";
...
def create_map(photo_data, direk_data, save_path, center_lat=None, center_lon=None, zoom_level=None):
        ...
        <script>
            var GizleGosterControl = L.Control.extend({{
                ...
                onAdd: function(map) {{
                    ...
                    this.button.title = 'Direkleri Gizle/Göster (Tıkla)';
                ...
                _toggle: function() {{
                    if (this.is_hidden) {{
                        this.button.innerHTML = '👁️';
                        this.button.title = 'Direkleri Gizle (Tıkla)';
                        this._show_direkler();
                        this.is_hidden = false;
                        console.log('👁️ Direkler gösteriliyor');
                    }} else {{
                        this.button.innerHTML = '👁️‍🗨️';
                        this.button.title = 'Direkleri Göster (Tıkla)';
                        this._hide_direkler();
                        this.is_hidden = true;
                ...
                _hide_direkler: function() {{
                    // Sadece cluster grubunu haritadan kaldır
                    ...
                    // Numara marker'larını da gizle (bunlar cluster içinde değil)
                    if (typeof direkler !== 'undefined' && direkler) {{
                        direkler.forEach(function(d) {{
                            if (d.numaraMarker && this._map.hasLayer(d.numaraMarker)) {{
                ...
                _show_direkler: function() {{
                    // Cluster grubunu tekrar haritaya ekle
                    if (typeof direklerCluster !== 'undefined' && direklerCluster && !this._map.hasLayer(direklerCluster)) {{
                        this._map.addLayer(direklerCluster);
                    }}
                    
                    // Numara marker'larını tekrar göster
                    if (typeof direkler !== 'undefined' && direkler) {{
                        direkler.forEach(function(d) {{
                            if (d.numaraMarker && !this._map.hasLayer(d.numaraMarker)) {{
                                this._map.addLayer(d.numaraMarker);
                                d.numaraMarker.bringToFront();
                            }}
                        }}, this);
                    }}
                }}
            ...
            function tumFotolariCikar() {{
                ...
                if (tumFotograflar.length > 0) {{
                    alert(`✅ Tüm fotoğraflar (${{tumFotograflar.length}} adet) direklerden çıkarıldı ve haritaya geri eklendi.`);
                }} else {{
    ...
    full_html = html_start + photo_markers + direk_markers + arama_kontrol + html_end
