...
class RaporEkrani(tk.Toplevel):
    ...
    def verileri_yukle(self):
        ...
        try:
            ...
            print(f"🔍 DEBUG: Fotoğraf Klasörleri sütunları: {list(df_fotograf.columns)}")
            
            # Direk Tipi sütununu kontrol et
            direk_tipi_sutunlari = ['Direk Tipi', 'Direk tipi', 'DİREK TİPİ', 'DirekTİPİ', 'Tipi']
            direk_tipi_sutun = None
            
            for sutun in direk_tipi_sutunlari:
                if sutun in df_fotograf.columns:
                    direk_tipi_sutun = sutun
                    print(f"✅ DEBUG: Direk Tipi sütunu bulundu: '{direk_tipi_sutun}'")
                    break
            
            if not direk_tipi_sutun:
                print("⚠️ DEBUG: Direk Tipi sütunu bulunamadı!")
                # Varsayılan değerlerle devam et
                direk_tipi_sutun = 'Direk Tipi'
                df_fotograf[direk_tipi_sutun] = ''
            
            # TRAFO DİREĞİ sütununu kontrol et
            trafo_sutun_adi = None
            trafo_sutun_adlari = ['Trafo Direği', 'Trafo direği', 'TRAFO DİREĞİ', 'Trafo', 'TrafoDireği', 'Trafo_Direği']
            
            for sutun in trafo_sutun_adlari:
                if sutun in df_fotograf.columns:
                    trafo_sutun_adi = sutun
                    print(f"✅ DEBUG: Trafo Direği sütunu bulundu: '{trafo_sutun_adi}'")
                    break
            
            if not trafo_sutun_adi:
                print("⚠️ DEBUG: Trafo Direği sütunu bulunamadı!")
                trafo_sutun_adi = 'Trafo Direği'
            ...
            # Hat bazında direk tipi sayımlarını hesapla
            hat_direk_tipi_sayilari = {}  # (İlçe, Hat) -> {'mustertek': X, 'og': Y, 'trafo': Z}
            
            for index, row in df_fotograf.iterrows():
                try:
                    aob = str(row.get('Aob', '')).strip()
                    hat_adi = str(row.get('Hat adı', '')).strip()
                    ...
                    # Direk tipine göre say
                    if 'MÜŞTEREK' in direk_tipi or 'MUSTEREK' in direk_tipi:
                        hat_direk_tipi_sayilari[hat_anahtari]['mustertek'] += 1
                    elif 'OG' in direk_tipi or 'OĞ' in direk_tipi:
                        hat_direk_tipi_sayilari[hat_anahtari]['og'] += 1
                    
                    # TRAFO DİREĞİ kontrolü
                    if 'TRAFO' in trafo_direk or 'EVET' in trafo_direk or 'VAR' in trafo_direk or 'X' in trafo_direk or '✓' in trafo_direk:
            ...
            print(f"📊 DEBUG: {len(hat_direk_tipi_sayilari)} hat için direk tipi sayımları hesaplandı")
            
            # Örnek göster
            for i, (anahtar, deger) in enumerate(list(hat_direk_tipi_sayilari.items())[:5]):
                print(f"   {anahtar}: Müşterek={deger['mustertek']}, OG={deger['og']}, Trafo={deger['trafo']}, Toplam={deger['toplam']}")

            # Excel dosyasını okumayı dene
            try:
                # Hat Özeti sayfasını oku
                ...
                messagebox.showerror("Hata", 
                                   f"Excel dosyası okunamadı veya 'Hat Özeti' sayfası bulunamadı:\n{str(e)}\n\n"
                                   f"Dosya: {self.excel_dosyasi}")
                self.destroy()
                return False
            
            # Fotoğraf Klasörleri sayfasını okumayı dene (MESAFE BİLGİLERİ İÇİN)
            try:
                df_fotograf = pd.read_excel(self.excel_dosyasi, sheet_name='Fotoğraf Klasörleri')
                ...
                # Fotoğraf Klasörleri sayfası yoksa uyarı ver ama devam et
                messagebox.showwarning("Uyarı", "'Fotoğraf Klasörleri' sayfası bulunamadı. Mesafe istatistikleri hesaplanamayacak.")
                df_fotograf = pd.DataFrame()

            # Hat bazında toplam mesafeyi hesapla
            ...
            toplam_hat = df_hat_ozet['Hat Adı'].nunique() if len(df_hat_ozet) > 0 and 'Hat Adı' in df_hat_ozet.columns else 0
            toplam_direk = len(df_fotograf) if not df_fotograf.empty else 0
            toplam_foto = df_fotograf['Fotoğraf Sayısı'].sum() if not df_fotograf.empty and 'Fotoğraf Sayısı' in df_fotograf.columns else 0
            ...
            # YENİ: Hat bazında ay verilerini sakla - FİLTRELEME İÇİN GEREKLİ
            self.hat_ay_detaylari = {}
            
            # Ay verilerini Fotoğraf Klasörleri sayfasından al ve HAT BAZINDA SAKLA
            ay_detay = {}  # Tüm veriler için ay detayı
            
            if not df_fotograf.empty and 'Ay' in df_fotograf.columns and 'Aob' in df_fotograf.columns and 'Hat adı' in df_fotograf.columns:
                for index, row in df_fotograf.iterrows():
                    ay = row.get('Ay')
                    aob = row.get('Aob')
                    ...
                    if pd.notna(ay):
                        try:
                            ...
                            if ay_str in ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']:
                                # 1. HAT BAZINDA AY DETAYI (filtreleme için)
                                ...
                                # 2. TOPLAM AY DETAYI (tüm veriler için)
            ...
            # Hat detay verilerini hazırla - FOTOĞRAF KLASÖRLERİ'NDEN DİREK TİPİ SAYIMLARI KULLAN
            hat_detay = []
            for index, row in df_hat_ozet.iterrows():
                # Sütun isimlerini kontrol et
                ilce_adi = row.get('İlçe', row.get('Aob', '')) if 'İlçe' in row else row.get('Aob', '')
                hat_adi = row.get('Hat Adı', row.get('Hat adı', ''))
                ...
                # 2. EĞER HALA 0 İSE: Direk sayısından tahmin et
                if foto_sayisi == 0 and direk_sayisi > 0:
                    foto_sayisi = direk_sayisi * 2  # Her direk için ortalama 2 fotoğraf

                print(f"📸 DEBUG: {hat_anahtari} - Fotoğraf: {foto_sayisi}, Bulgu: {bulgu_sayisi}")
                
                print(f"🔍 DEBUG: {hat_anahtari} - "
                      f"Müşterek: {mustertek_direk}, "
                ...
                hat_detay.append({
                    ...
                    'og_direk': og_direk,  # Direk Tipi'ndeki OG + Trafo Direği
                    ...
                    'trafo_direk': direk_tipi_istatistikleri['trafo'],  # Sadece Trafo sayısı
            ...
            print(f"🎯 DEBUG: Rapor verileri hazır")
            print(f"📊 DEBUG: Toplam Müşterek Direk: {toplam_mustertek}")
            print(f"📊 DEBUG: Toplam OG Direk (Trafo dahil): {toplam_og}")
            print(f"📊 DEBUG: Toplam Trafo Direği: {toplam_trafo}")
            print(f"📊 DEBUG: Toplam OG Direk (saf): {toplam_og_saf}")
            print(f"📅 DEBUG: Hat bazında ay verileri hazır - {len(self.hat_ay_detaylari)} hat için")
            ...
            # Öncelik verilerini yükle (RaporEkrani açıldığında otomatik yükle)
...
class AnaUygulama:
    ...
    def fotograf_klasorleri_listele(self):
        ...
        # 1. ÖNCE MASADÜSTÜNDEKİ DRONE KLASÖRÜNDEN EXCEL DOSYALARINI OTOMATİK BUL
        desktop_path = os.path.join(os.path.expanduser("~"), "Desktop")
        drone_klasoru = os.path.join(desktop_path, "Drone")
        
        # Excel dosya adlarını tanımla
        ...
        if os.path.exists(drone_klasoru):
            ...
            if not self.direk_musterek_yolu or not self.direk_og_yolu:
                self.log_mesaj_ekle("📂 Klasördeki tüm Excel dosyaları taranıyor...")
                for dosya in os.listdir(drone_klasoru):
                    if dosya.lower().endswith(('.xlsx', '.xls')):
                        dosya_yolu = os.path.join(drone_klasoru, dosya)
                        
                        # Dosya adına göre türünü belirle
                        dosya_adi_lower = dosya.lower()
                        
                        if not self.direk_musterek_yolu and any(keyword in dosya_adi_lower for keyword in ['musterek', 'müşterek']):
                            self.direk_musterek_yolu = dosya_yolu
                            self.log_mesaj_ekle(f"✅ Müşterek direk Excel'i bulundu: {dosya}")
                        
                        if not self.direk_og_yolu and any(keyword in dosya_adi_lower for keyword in ['og', 'og_direk', 'ogdirek', 'TRAFO DİREĞİ']):
                            self.direk_og_yolu = dosya_yolu
                            self.log_mesaj_ekle(f"✅ OG direk Excel'i bulundu: {dosya}")
        ...
        if not self.direk_musterek_yolu and not self.direk_og_yolu:
            ...
            # OTOMATİK BULUNDU, KULLANICIYA BİLGİ VER
            bulunan_dosyalar = []
    ...
    def excel_islemi_tamamla(self, foto_klasorleri, atlanan_klasorler, hat_sureleri=None, hat_tarihsiz_istatistikleri=None):
        ...
        try:
            ...
            # 4. AŞAMA: Mesafe bilgilerini hesapla
            self.log_mesaj_ekle("📏 Mesafe hesaplamaları yapılıyor...")
            mesafe_sonuclari = self.mesafe_hesapla(foto_klasorleri, koordinat_bilgileri)
            ...
            if foto_klasorleri:
                ...
                basliklar = [
                    ...
                    'Toplam Gün', 'Direk Tipi', 'Trafo Direği', 'LAT', 'LON', 'Mesafe (m)', 
    ...
    def mesafe_hesapla(self, foto_klasorleri, koordinat_bilgileri):
        """Her direk için bir önceki direkle arasındaki mesafeyi METRE cinsinden hesaplar - TRAFO DİREĞİ özel durumu ve 750m limiti ile"""
        mesafe_sonuclari = {}
        
        try:
            mesafe_sonuclari.clear()
            self.log_mesaj_ekle("🔄 Mesafe hesaplamaları yapılıyor (TRAFO DİREĞİ özel durumu ve 750m limiti ile)...")
            ...
            for hat_anahtari, klasor_listesi in hat_gruplari.items():
                ...
                for i, klasor in enumerate(sirali_klasorler):
                    ...
                    # ÜÇLÜ ANAHTAR: (Hat-Adı, SıraNo, DirekNo)
                    ...
                    if i == 0:
                        ...
                        if (onceki_lat is not None and onceki_lon is not None and 
                            ...
                            try:
                                ...
                                if mesafe_metre > 750:
                                    mesafe_deger = ""
                                    self.log_mesaj_ekle(f"🏭 {aob}-{hat_adi}-{sira_no}-{temiz_direk_no}: TRAFO DİREĞİ - {mesafe_metre:.2f} m (750m üzeri - boş)")
                                else:
                                    mesafe_deger = float(mesafe_metre)
                                    self.log_mesaj_ekle(f"🏭 {aob}-{hat_adi}-{sira_no}-{temiz_direk_no}: TRAFO DİREĞİ - {mesafe_deger:.2f} m (önceki direkten)")
                            except Exception as e:
                                mesafe_deger = "Hesaplanamadı"
                                self.log_mesaj_ekle(f"⚠️ Trafo mesafe hesaplama hatası {uc_anahtar}: {str(e)}")
                        else:
                            mesafe_deger = "Koordinat yok"
                            self.log_mesaj_ekle(f"⚠️ {uc_anahtar} (TRAFO): Koordinat eksik")
                    
                    elif onceki_direk_no and onceki_direk_no in trafo_direkleri:
                        # ÖNCEKİ DİREK TRAFO İSE: Bu direğe mesafe yazma
                        mesafe_deger = ""
                        self.log_mesaj_ekle(f"⏭️ {aob}-{hat_adi}-{sira_no}-{temiz_direk_no}: Önceki direk trafo - mesafe atlandı")
                    
                    elif (onceki_lat is not None and onceki_lon is not None and 
                          simdiki_lat is not None and simdiki_lon is not None):
                        # NORMAL DURUM: Önceki direkle arasındaki mesafeyi hesapla
                        try:
                            mesafe_metre = self.koordinat_mesafe_hesapla(onceki_lat, onceki_lon, simdiki_lat, simdiki_lon)
                            # 750m KONTROLÜ
                            if mesafe_metre > 750:
                                mesafe_deger = ""
                                self.log_mesaj_ekle(f"📏 {aob}-{hat_adi}-{sira_no}-{temiz_direk_no}: {mesafe_metre:.2f} m (750m üzeri - boş)")
                            else:
                                mesafe_deger = float(mesafe_metre)
                                self.log_mesaj_ekle(f"📏 {aob}-{hat_adi}-{sira_no}-{temiz_direk_no}: {mesafe_deger:.2f} m")
                        except Exception as e:
                            mesafe_deger = "Hesaplanamadı"
            ...
            trafo_mesafe_sayisi = sum(1 for k, v in mesafe_sonuclari.items() 
                                    if v and v != "Koordinat yok" and v != "Hesaplanamadı" and 
                                    k[3] in trafo_direkleri)
            atlanan_mesafe_sayisi = sum(1 for v in mesafe_sonuclari.values() if v == "")
            buyuk_mesafe_sayisi = sum(1 for v in mesafe_sonuclari.values() if isinstance(v, (int, float)) and v > 750)
            
            self.log_mesaj_ekle(f"🏭 Toplam {toplam_trafo} trafo direği tespit edildi")
            self.log_mesaj_ekle(f"📏 {trafo_mesafe_sayisi} trafo direğine mesafe yazıldı")
            self.log_mesaj_ekle(f"⏭️ {atlanan_mesafe_sayisi} mesafe trafo nedeniyle atlandı")
            self.log_mesaj_ekle(f"🚫 {buyuk_mesafe_sayisi} mesafe 750m üzeri olduğu için boş bırakıldı")
                    
        except Exception as e:
            self.log_mesaj_ekle(f"❌ Mesafe hesaplama hatası: {str(e)}")
        
        return mesafe_sonuclari
    ...
    def direk_tipi_bilgilerini_al(self, veri, tespit_verileri=None):
        """ÇOK HIZLI versiyon - pandas merge kullan ve tespit bilgilerini de al"""
        try:
            # 1. Ana veriden direk numaralarını hazırla - SET kullan
            ana_direk_set = set()
            for row_data in veri:
                direk_no = str(row_data.get('Direk No', ''))
                temiz_no = self.direk_no_temizle(direk_no)
            ...
            # 3. OG direk Excel'inden TRAFO DİREĞİ bilgilerini al
            if self.direk_og_yolu and os.path.exists(self.direk_og_yolu):
                try:
                    df_og = pd.read_excel(self.direk_og_yolu)
                    ...
                    if direkno_col and tip_col:
                        for index, row in df_og.iterrows():
                            ...
                            if temiz_no and temiz_no in ana_direk_set:
                                # "TRAFO DIREGI" kontrolü (büyük/küçük harf duyarsız)
                                if "TRAFO" in tip_deger or "TRAFO DİREĞİ" in tip_deger.upper():
                                    trafo_direkleri[temiz_no] = "TRAFO DİREĞİ"
                            
                        self.txt_sonuc.insert(tk.END, f"✓ OG Excel'inden {len(trafo_direkleri)} trafo direği bulundu\n")
                        
                except Exception as e:
                    self.txt_sonuc.insert(tk.END, f"⚠️ OG Excel trafo bilgisi okuma hatası: {str(e)}\n")

            # 4. Müşterek direkleri HIZLI şekilde bul
            if self.direk_musterek_yolu and os.path.exists(self.direk_musterek_yolu):
                try:
                    df_musterek = pd.read_excel(self.direk_musterek_yolu)
                    ...
                    if direkno_col:
                        ...
                        self.txt_sonuc.insert(tk.END, f"✓ Müşterek direk Excel'inde eşleşen: {len(eslesenler)} direk\n")
                        
                except Exception as e:
                    self.txt_sonuc.insert(tk.END, f"⚠️ Müşterek Excel okuma hatası: {str(e)}\n")

            # 5. OG direkleri HIZLI şekilde bul (trafo olmayanlar)
            if self.direk_og_yolu and os.path.exists(self.direk_og_yolu):
                try:
                    df_og = pd.read_excel(self.direk_og_yolu)
                    ...
                    if direkno_col:
                        ...
                        # Trafo direklerini çıkararak OG direkleri bul
                        ...
                        self.txt_sonuc.insert(tk.END, f"✓ OG direk Excel'inde eşleşen: {len(eslesenler)} direk\n")
            ...
            self.txt_sonuc.insert(tk.END, f"📊 Toplam {len(direk_tipleri)} direk tipi bilgisi bulundu\n")
            self.txt_sonuc.insert(tk.END, f"📊 {len(trafo_direkleri)} trafo direği bulundu\n")
            self.txt_sonuc.insert(tk.END, f"📊 Tespit bilgisi bulunan: {len(tespit_bilgileri)} direk\n")
            
            # Direk tipi, trafo bilgisi ve tespit bilgilerini birleştir
            sonuc = {
